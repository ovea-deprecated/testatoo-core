...
/**
 * This class is a matcher (written based on hamcrest possibilites) to test the content of textfield.
 *
...
public class ContainsText extends TypeSafeMatcher<TextField> {
    ...
    /**
     * Uses the matcher ContainsText to compare the content of a given textfield to the expected values of the matcher,
     * using the syntax "containsText("xxx")" or "containsExactlyText("xxx")"
     *
     * @param textField the textField
     * @return True if the textfield contains (contains exactly for second syntax) the values of the matcher ContainsText
     */
    public boolean matchesSafely(TextField textField) {
        if (exactly) {
            return text.equals(textField.value());
        }
        String patternStr = text;
        Pattern pattern = Pattern.compile(patternStr);
        java.util.regex.Matcher matcher = pattern.matcher(textField.value());
        return matcher.find();
    }
     ...
     * @param text    text we want to know if they are (or are exactly) in the textfield
     */
     ...
     *
     * @param text the text to compare to the content of the textfield
     * @return a new ContainsText matcher
     */
    @Factory
    public static Matcher<TextField> contains(String text) {
     ...
     */
    @Factory
    public static Matcher<TextField> containsExaclty(String text) {
